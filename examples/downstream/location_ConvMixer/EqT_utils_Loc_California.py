    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            if dataset.attrs['data_category'] != 'noise':
                ...
                latdiff = dataset.attrs['station_lat']  - dataset.attrs['event_lat']
                londiff = dataset.attrs['station_long'] - dataset.attrs['event_long'] 
                dep = float(dataset.attrs['depth'])
                

                if dep<0:
                    dep=0

                dep = (dep - self.min_d) / (self.max_d - self.min_d)
