...
# =============================================================================
#
# Adapted from EQTransformer:
#   https://github.com/smousavi05/EQTransformer/tree/master/EQTransformer/core
#
# This is a modified version used for the EQCCT P-wave picking pipeline.
#
# What this file provides
# -----------------------
# 1) DataGenerator
#    - Keras Sequence generator for training
#    - Reads waveform windows from an HDF5 dataset
#    - Performs optional augmentation (shift, noise, gaps, channel drop, etc.)
#    - Produces labels for P-wave picking (triangle label supported here)
#
# 2) DataGeneratorTest
#    - Lightweight generator for inference/testing (no augmentation)
#
# 3) data_reader
#    - Utility to preload data into memory (optionally with augmentation)
#
# 4) picker + peak detection
#    - Converts predicted probability traces into discrete P-pick indices
#    - Supports uncertainty estimation (if std is provided)
#
# 5) Training helpers
#    - f1 metric
#    - learning rate schedule
#
# Data format expectations (HDF5)
# ------------------------------
# For DataGenerator / DataGeneratorTest:
# - file_name: path to HDF5
# - Each trace is accessed via: fl.get(str(ID))
# - Each dataset is assumed to contain waveform at dataset['data'] (shape Lx3)
#
# Required attributes for earthquake traces (data_category != "noise"):
# - p_arrival_sample (int)
# - s_arrival_sample (int)
# - coda_end_sample  (int)
# - snr_db           (array-like length 3 or scalar)
#
# For noise traces (data_category == "noise"):
# - waveform data exists, but pick attributes may not.
#
# Output shapes
# -------------
# X  : (B, dim, 3)     input waveforms
# y2 : (B, dim, 1)     P-phase label trace (triangle or zeros for noise)
#
# =============================================================================
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            try:
                ...
                if self.label_type  == 'triangle' and dataset.attrs['data_category'] != 'noise':                      
                    sd = None    
                    if spt and sst: 
                        sd = sst - spt                      
